...
"""
"""
Modified output for Pipeline by Kimhun
"""
...
#Modified for local relative import by Kimhun
import os, sys, importlib

# → 1. path pointing to .../prodigy/src  (the *parent* of prodigy_prot)
repo_root = os.path.abspath(
    os.path.join(os.path.dirname(__file__), os.pardir)  # .../prodigy/src
)

# Insert at the front so it wins over site-packages
if repo_root not in sys.path:
    sys.path.insert(0, repo_root)

# Force-reload prodigy_prot so the repo copy replaces any earlier import
import prodigy_prot
importlib.reload(prodigy_prot)
...
class Prodigy:
    ...
    def print_prediction(self, outfile="", quiet=False):
        ...
        if quiet:
            # Keep the existing quiet behavior:
            handle.write(f"{self.structure.id},{self.ba_val:8.3f}\n")
        else:
            # Print everything on one line, comma-separated:
            line = ( f"output:"
                f"{len(self.ic_network)},"
                f"{self.bins['CC']},"
                f"{self.bins['CP']},"
                f"{self.bins['AC']},"
                f"{self.bins['PP']},"
...
#python /Users/mac/Desktop/AF3/AF3/prodigy/src/prodigy_prot/Modified_predict_IC.py --temperature 37 /Users/mac/Desktop/AF3/folds_2025_05_22_07_31/5b4x_all/pdb/fold_5b4x_all_model_0.pdb